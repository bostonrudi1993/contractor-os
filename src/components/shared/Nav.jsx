    ...
    <>
      ...
      <div style={{position:"fixed",top:0,left:0,height:"100%",width:260,background:"#0d0d0d",borderRight:"1px solid #1e1e1e",zIndex:201,transform:navOpen?"translateX(0)":"translateX(-100%)",transition:"transform 0.25s cubic-bezier(0.4,0,0.2,1)",display:"flex",flexDirection:"column",overflowY:"auto"}}>
                  ...
                  <div style={{background:"#080808",borderLeft:`3px solid ${accent}22`,marginLeft:20}}>
                    {subPages.map(sp => {
                      ...
                      return (
                        <button
                          key={sp.id}
                          onClick={()=>{onNav(id);setSubScreen&&setSubScreen(sp.id);setNavOpen(false);}}
        ...
        <div style={{flexShrink:0}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 18px",borderTop:"1px solid #1e1e1e",background:"#080808"}}>
            <div style={{fontSize:9,color:"#444",letterSpacing:"0.1em",textTransform:"uppercase"}}>{seg.icon} {seg.label}</div>
            {TIERS && currentTier && (
